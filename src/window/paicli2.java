	 ...
	 // 中心的panel显示拍卖的面板		
			...
			JPanel jp11=new JPanel(){
	            public void paintComponent(Graphics g) {
	                ...
	                ImageIcon ii = new ImageIcon("D:/PS修图/原图/日本街道2.JPG");//演示的时候把这个改回strpath，记得秒删              
	                g.drawImage(ii.getImage(), 0, 0, getWidth(), getHeight(), ii.getImageObserver());
	            }
	        };
	        JPanel jp111=new JPanel();
	        jp111.setLayout(new GridLayout(3,1));
			JLabel jl11=new JLabel("叙述");
			JLabel jl111=new JLabel("您是"+id+"号");
			jl111.setBorder(border7);
			jl111.setFont(fg);
			JButton jb11=new JButton("增加10元");
			jb11.setFont(fg);
			jp111.add(jl11);jp111.add(jl111);jp111.add(jb11);
			...
			try {
			   ...
			   BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(socket.getInputStream()));
			      try {
			          ...
			          jl111.setText("您是"+i+"号");
			          ActionListener acd=new acd1(i,socket);
					  jb11.addActionListener(acd);
						ActionListener infor=new infor1(i,socket);
						jbdown.addActionListener(infor);
			          while ((msg = bufferedReader.readLine()) != null) {
			            	if(msg==("1")||msg=="2") {
			            		String ss="当前价格为: "+(price+10)+"元";
			        			String ss2="当前持有者为"+msg+"号";
			        			jl2=new JLabel(ss);
			        			jl2.setForeground(Color.red);
			        			jl3=new JLabel(ss2);
			        			jl3.setForeground(Color.red);
...
class acd2 implements ActionListener{
	int id;Socket so;private PrintWriter printWriter;
public acd2(int i,Socket so) {
	id=i;
	this.so=so;
	try {
	...
	public void actionPerformed(ActionEvent e) {
	    ...
	    paicli2.price+=10;
	    paicli2.id++;;
	    String show="当前价格为: "+paicli2.price+" 元";
	    String show2="持有者为"+paicli2.id+"号";
        paicli2.jl2.setText(show);
        paicli2.jl3.setText(show2);
		paicli2.jp2.validate();
	}	
}

class infor2 implements ActionListener{
	int id;Socket so;private PrintWriter printWriter;
public infor2(int n,Socket s) {	
	id=n;
	so=s;
	try {
		printWriter = new PrintWriter(new OutputStreamWriter(so.getOutputStream()), true);
