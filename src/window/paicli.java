...
public class paicli {
	...
	public static void main(String args[]) throws IOException {
	 ...
	 // 中心的panel显示拍卖的面板		
			...
			JLabel jl11=new JLabel("欢迎，本件商品由XX制成！");
...
class acd1 implements ActionListener{
	int id;Socket so;private PrintWriter printWriter;
public acd1(int i,Socket so) {
	id=i;
	this.so=so;
	try {
...
class infor1 implements ActionListener{
	int id;Socket so;private PrintWriter printWriter;
public infor1(int n,Socket s) {	
	id=n;
	so=s;
	try {
		printWriter = new PrintWriter(new OutputStreamWriter(so.getOutputStream()), true);
